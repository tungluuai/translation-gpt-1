...
# url_weight = "https://drive.google.com/file/d/1gpQX_9Mt6dlJJBwJPvmtoReMhfUPkcbF/view?usp=sharing"
url_weight = "flaskr/model_translation/data/transformer_model.pth"
transformer.load_state_dict(torch.load(url_weight, map_location=torch.device('cpu'))) # add only run with cpu
...
def translate(input_sentence, transformer):
  ...
  vi_senetence = tokenizer.decode(dec_input_ids[0, 1:-1])
  return vi_senetence

def translation(text):
  return translate(text, transformer)